...
module Spree
  class OklinkController < StoreController
  	include HTTParty
	...
	def redirect
		...
		# Create oklink button code
		secret_token = SecureRandom.base64(30)
		button_params = { 
				:name => "Order #%s" % order.number,
				:price => (float)order.total,
				:price_currency => order.currency,
				:callback_url => spree_oklink_callback_url(:payment_method_id => params[:payment_method_id], :secret_token => secret_token),
				:success_url => spree_oklink_success_url(:payment_method_id => params[:payment_method_id], :order_num => order.number),
				}

        key = payment_method.preferred_api_key
		secret = payment_method.preferred_api_secret				
		client = make_oklink_request :post, "/buttons", button_params
		...
		if code
			...
			transaction = OklinkTrancation.new
			...
			redirect_to "https://oklink.com/merchant/mPayOrderStemp1.do?buttonid=%1$s" % [code]
		else
			redirect_to edit_order_checkout_url(order, :state => 'payment'),
                    :notice => Spree.t(:spree_oklink_checkout_error)
		end
	end

	def callback

		# Download order information from oklink (do not trust sent order information)
		cb_order_id = params["order"]["id"]
		cb_order = make_oklink_request :get, "/orders/%s" % cb_order_id, {}
	...
	def cancel
		...
		# cancel all Oklink payments (it is unlikely there will be more than one)
		...
		redirect_to edit_order_checkout_url(order, :state => 'payment'),
			:notice => Spree.t(:spree_oklink_checkout_cancelled)
	end
	...
	def payment_method
		...
		if !(m.is_a? Spree::PaymentMethod::Oklink)
			raise "Invalid payment_method_id"
		end
		m
	end

	# the oklink-ruby gem is not used because of its dependence on incompatible versions of the money gem
	def make_oklink_request verb, path, options

		if key.nil? || secret.nil?
			raise "Please enter an API key and secret in Spree payment method settings"
		end

		base_uri = "https://www.oklink.com/api/v1"
